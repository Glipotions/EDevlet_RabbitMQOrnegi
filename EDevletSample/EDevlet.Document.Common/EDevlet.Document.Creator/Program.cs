...
namespace EDevlet.Document.Creator
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                var modelReceived = Encoding.UTF8.GetString(ea.Body.ToArray());
                var model = JsonConvert.DeserializeObject<CreateDocumentModel>(modelReceived);

                Task.Delay(5000).Wait();

                model.Uri = "http://www.turkiye.gov.tr/docs/x.pdf";
                Console.WriteLine($"Received Data Url : {model.Uri}");
